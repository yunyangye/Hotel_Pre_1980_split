...
#start the measure
class ChangeInsulationRValueForRoofs < OpenStudio::Ruleset::ModelUserScript

  #define the name that a user will see
  def name
    return "Change R-value of Insulation for Roofs to a Specific Value"
  end
  
  # human readable description
  def description
    return "This measure aims to change R-value of roofs."
  end

  # human readable description of modeling approach
  def modeler_description
    return "Modify the R-value of roofs."
  end
  ...
  def arguments(model)
    ...
    r_value_roof = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("r_value_roof",true)
    r_value_roof.setDisplayName("Insulation R-value (ft^2*h*R/Btu).")
    r_value_roof.setDefaultValue(30.0)
    args << r_value_roof
  ...
  def run(model, runner, user_arguments)
    ...
    r_value_roof = runner.getDoubleArgumentValue("r_value_roof",user_arguments)
    ...
    if r_value_roof < 0 or r_value_roof > 500
      runner.registerError("The requested roof insulation R-value of #{r_value_roof} ft^2*h*R/Btu was above the measure limit.")
      return false
    elsif r_value_roof > 60
      runner.registerWarning("The requested roof insulation R-value of #{r_value_roof} ft^2*h*R/Btu is abnormally high.")
    elsif r_value_roof < min_expected_r_value_ip
      runner.registerWarning("The requested roof insulation R-value of #{r_value_roof} ft^2*h*R/Btu is abnormally low.")
    end
    ...
    r_value_si = unit_helper(r_value_roof, "ft^2*h*R/Btu","m^2*K/W")
    ...
    exterior_surface_constructions.each do |exterior_surface_construction|
      ...
      if not thermal_resistance_values.max > unit_helper(min_expected_r_value_ip, "ft^2*h*R/Btu","m^2*K/W")
        ...
        if found_material == false
          ...
          new_material.setName("#{max_thermal_resistance_material.name.to_s}_R-value #{r_value_roof} (ft^2*h*R/Btu)")
          materials_hash[max_thermal_resistance_material.name.to_s] = new_material
          final_construction.eraseLayer(max_thermal_resistance_material_index)
          final_construction.insertLayer(max_thermal_resistance_material_index,new_material)
    ...
    runner.registerFinalCondition("The existing insulation for roofs was changed to R-#{r_value_roof}. This was applied to #{neat_numbers(affected_area_ip,0)} (ft^2) across #{final_string.size} roof constructions: #{final_string.sort.join(", ")}.")
...
ChangeInsulationRValueForRoofs.new.registerWithApplication
